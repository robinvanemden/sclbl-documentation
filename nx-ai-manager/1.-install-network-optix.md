...
* Download the [x64 Ubuntu Nx Meta server](https://meta.nxvms.com/download/linux) installer (metavms-server-6.x.x.x-linux\_x64.deb). \
  ...
  *   You can usually use wget or curl in an SSH session to download the installer files directly to the server.
      ...
      ```sh
      # For example the the 39503 build x64 architecture
      wget https://updates.networkoptix.com/metavms/39503/linux/metavms-server-6.0.0.39503-linux_x64.deb

      # Or the same example 39503 version with the ARM64 architecture
      wget https://updates.networkoptix.com/metavms/39503/arm/metavms-server-6.0.0.39503-linux_arm64.deb
      ```
